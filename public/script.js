...
window.addEventListener('load', () => {
  ...
  document
    ...
    .setAttribute('content', 'lightgrey')
  if (!skip) {
...
document.querySelector('#random-btn').addEventListener('click', (e) => {
  ...
  document
    .querySelector('[name="theme-color"]')
    .setAttribute('content', 'darkgreen')
  handelRandom()
})
...
function fillBingo(player, num) {
  num = num > 5 ? 5 : num
  for (let i = 1; i <= num; i++) {
    document.querySelector(`#${player}-letter-${i}`).style.backgroundColor =
      'teal'
...
function handelRandom() {
  if (name != '' && name != 'null' && name != undefined) {
    ...
    document.querySelector('.share').style.display = 'none'
    document.querySelector('.invite-msg').style.display = 'none'
    document.querySelector('.player-name').innerHTML = name
...
socket.on('other-player-joined', (data) => {
  ...
  document.querySelector(
    '#opponent-avatar'
  ).src = `/images/avatar${data.avatar}.png`
})
...
socket.on('hosted', (data) => {
  ...
  document.getElementById(
    'whatsapplink'
  ).href = `whatsapp://send?text=${name} has invited you to join Bingo Online Multiplayer Game.%0D%0A %0D%0AClick the link to join game %0D%0A${location.protocol}//${location.host}/join/${data.roomid}`
})
